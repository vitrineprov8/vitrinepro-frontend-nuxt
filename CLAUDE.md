...
- **B2/B7 validado 2026-07-05 até o ponto que não exige senha:** `forgot-password` disparado de verdade pela UI para `testeia@getnada.com` → tela "verifique seu e-mail" confirmada; `/redefinir-senha/[token]` e `/convite/[token]` renderizam corretamente o estado de link inválido contra o backend real (404, não mock). **Falta**: Andres digitar a nova senha a partir do e-mail recebido, e testar um convite de time de verdade (precisa de conta plano TEAM/ENTERPRISE — não existe ainda em `qa-test-accounts.json`).
