...
// T-H03 — Minhas Vagas (ATS próprio). Dados reais de GET /vagas/me + /vagas/me/usage.
...
const { data: usage, refresh: refreshUsage } = await useAsyncData('hunter-vagas-usage', () =>
...
const { data: resp, pending, refresh } = await useAsyncData('hunter-vagas', () =>
...
<template>
  <div class="mv">
    <header class="mv__header">
      ...
      <UiButton @click="navigateTo('/app/hunter/vagas/nova')">Nova vaga</UiButton>
    </header>
    ...
    <div v-else-if="vagas.length" class="mv__table-wrap">
      <table class="mv__table">
        <thead>
          <tr><th>Vaga</th><th>Status</th><th>Candidatos</th><th>Publicada</th><th></th></tr>
        </thead>
        <tbody>
          <tr v-for="v in vagas" :key="v.id" class="mv__row" @click="navigateTo(`/app/hunter/vagas/${v.id}`)">
            <td>
            ...
            <td class="mv__actions" @click.stop>
              <details class="menu">
                ...
                <div class="menu__list">
                  <button @click="navigateTo(`/app/hunter/vagas/${v.id}`)">Ver pipeline</button>
                  <button @click="navigateTo(`/app/hunter/vagas/${v.id}/editar`)">Editar</button>
...
.mv__table { width: 100%; border-collapse: collapse; font-size: var(--text-14); min-width: 640px; }
