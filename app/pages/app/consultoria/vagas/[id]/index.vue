...
// T-T03/T-T04 — Pipeline de uma vaga do time (Workspace Consultoria).
// Mesmo kanban do T-E04 (Empresa) + aba Hunters, com o header mostrando
// Cliente e Responsável da vaga.
...
const { data: vaga } = await useAsyncData(`consultoria-pipe-vaga-${vagaId}`, async () => {
  const list = await api.get<PaginatedResult<Vaga>>('/vagas/me', { limit: 100 }).catch(() => null)
  return list?.data.find(v => v.id === vagaId) ?? null
})

const { data: template } = await useAsyncData('consultoria-pipe-template', () =>
...
const { data: appsResp, refresh } = await useAsyncData(`consultoria-pipe-apps-${vagaId}`, () =>
...
const responsavelNome = computed(() => {
  const a = vaga.value?.assignedTo
  return a ? `${a.firstName ?? ''} ${a.lastName ?? ''}`.trim() || 'Membro' : null
})
...
const { data: hunterInterestsResp, refresh: refreshHunterInterests } = await useAsyncData(`consultoria-pipe-hunters-${vagaId}`, () =>
  ...
  <div class="pipe">
    <header class="pipe__header">
      <div class="pipe__head-left">
        <NuxtLink to="/app/consultoria/vagas" class="pipe__back">← Vagas do time</NuxtLink>
        ...
        <div class="pipe__pills">
          ...
          <span v-if="vaga?.company" class="pipe__pill">Cliente: {{ vaga.company.name }}</span>
          <span v-if="responsavelNome" class="pipe__pill">Responsável: {{ responsavelNome }}</span>
      ...
      <div class="pipe__head-actions">
        ...
        <UiButton variant="secondary" size="sm" @click="navigateTo(`/app/consultoria/vagas/${vagaId}/editar`)">Editar</UiButton>
      </div>
    </header>
